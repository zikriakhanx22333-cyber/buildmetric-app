...
import { FootingInputs, FootingResults } from '../../types';
import { calculateFooting } from '../../utils/calculatorLogic';
import { Copy, Check, Printer, RotateCcw, ChevronDown, ChevronUp, Boxes } from 'lucide-react';

export const FootingCalculator: React.FC = () => {
  ...
  const results: FootingResults = calculateFooting(inputs);

  const handleChange = (field: keyof FootingInputs, value: any) => {
  ...
  const handleCopy = () => {
    const text = `
BuildMetric - Footing Concrete Calculation Results:
--------------------------------------------------
Number of Footings: ${inputs.numberOfFootings}
Dimensions per Footing: ${inputs.length} × ${inputs.width} × ${inputs.depth} ${inputs.unit}
Mix Grade: ${inputs.mixRatio}
Total Wet Concrete Volume: ${results.totalWetVolumeCft} CFT (${results.totalWetVolumeCum} m³)
    ...
    <div className="space-y-6">
      <div className="bg-[#0F2D5C] text-white p-6 rounded-2xl shadow-md border-b-4 border-[#F4B400]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-bold">Footing Concrete & Mix Calculator</h2>
            <p className="text-blue-100 text-sm">Compute concrete volume, cement bags, sand, and aggregate for foundation footings</p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                min="1"
                value={inputs.numberOfFootings}
                onChange={e => handleChange('numberOfFootings', Math.max(1, Number(e.target.value)))}
