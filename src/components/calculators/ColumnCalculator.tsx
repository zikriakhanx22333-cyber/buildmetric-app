...
import { ColumnInputs, ColumnResults } from '../../types';
import { calculateColumn } from '../../utils/calculatorLogic';
...
export const ColumnCalculator: React.FC = () => {
  const [inputs, setInputs] = useState<ColumnInputs>({
    ...
    length: 18, // 18 inches
    height: 10, // 10 feet
    unit: 'in', // for width and length, height in ft
    mixRatio: 'M25',
    wastagePercent: 5
  });
  ...
  // Adapt inputs for calculation logic
  const normalizedInputs: ColumnInputs = {
    ...inputs,
    // Width & Length in inches -> convert to feet for calculation logic
    width: inputs.unit === 'in' ? inputs.width / 12 : inputs.width,
    length: inputs.unit === 'in' ? inputs.length / 12 : inputs.length,
    height: inputs.height,
    unit: 'ft'
  };

  const results: ColumnResults = calculateColumn(normalizedInputs);

  const handleChange = (field: keyof ColumnInputs, value: any) => {
  ...
  const handleCopy = () => {
    const text = `
BuildMetric - Column Concrete Calculation Results:
-------------------------------------------------
Number of Columns: ${inputs.numberOfColumns}
Column Cross-Section: ${inputs.width} × ${inputs.length} ${inputs.unit}
Column Height: ${inputs.height} ft
    ...
    <div className="space-y-6">
      <div className="bg-[#0F2D5C] text-white p-6 rounded-2xl shadow-md border-b-4 border-[#F4B400]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-bold">Column Concrete & Material Calculator</h2>
            <p className="text-blue-100 text-sm">Calculate concrete volume, cement bags, sand, and aggregate for RCC columns</p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={e => handleChange('numberOfColumns', Math.max(1, Number(e.target.value)))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value="M25">M25 (1 : 1 : 2) Recommended</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">
