...
import { SlabInputs, SlabResults } from '../../types';
import { calculateSlab } from '../../utils/calculatorLogic';
...
export const SlabCalculator: React.FC = () => {
  ...
  const results: SlabResults = calculateSlab(inputs);

  const handleChange = (field: keyof SlabInputs, value: any) => {
  ...
  const handleCopy = () => {
    const text = `
BuildMetric - Roof Slab Concrete Calculation Results:
---------------------------------------------------
Slab Dimensions: ${inputs.length} × ${inputs.width} ${inputs.lengthUnit} (${inputs.thickness} ${inputs.thicknessUnit} thickness)
Mix Grade: ${inputs.mixRatio}
Total Wet Concrete Volume: ${results.wetVolumeCft} CFT (${results.wetVolumeCum} m³)
    ...
    <div className="space-y-6">
      <div className="bg-[#0F2D5C] text-white p-6 rounded-2xl shadow-md border-b-4 border-[#F4B400]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-bold">Roof Slab Concrete & Material Calculator</h2>
            <p className="text-blue-100 text-sm">Calculate wet/dry concrete, cement bags, sand, and aggregate for house roof slabs</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={inputs.thicknessUnit}
                onChange={e => handleChange('thicknessUnit', e.target.value)}
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 font-semibold focus:ring-2 focus:ring-[#0F2D5C] focus:bg-white outline-none"
              >
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">
                Thickness ({inputs.thicknessUnit})
              </label>
              <input
                type="number"
                min="0.1"
          ...
          <div>
            ...
            <div className="space-y-3 bg-slate-800/60 p-4 rounded-xl border border-slate-800 text-sm">
              <div className="flex justify-between py-1 border-b border-slate-800">
                <span className="text-slate-400">Wet Concrete Vol:</span>
                <span className="font-bold text-white">{results.wetVolumeCft} CFT ({results.wetVolumeCum} m³)</span>
              </div>
